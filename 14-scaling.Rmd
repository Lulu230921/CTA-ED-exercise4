...
https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/EL3KYD
...
Take a sample from these data to speed up the runtime of some of the analyses. 
...
```{r}
# compress the document-feature matrix at the newspaper level 
kaneko_dfm <- dfm_group(kaneko_dfm, groups = Newspaper)

# remove words not used by two or more newspapers：
kaneko_dfm <- dfm_trim(kaneko_dfm, 
                               min_docfreq = 2, docfreq_type = "count")
...
#### estimate the Wordfish model ####
set.seed(123L)
kaneko_dfm_results <- textmodel_wordfish(kaneko_dfm, 
                                             sparse = TRUE)
```
